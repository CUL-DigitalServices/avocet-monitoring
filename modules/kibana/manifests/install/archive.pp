class kibana::install::archive ($install_config, $stats_dir = '/opt/kibana') {
    ...
    exec { 'remove_original_kibana_archive':
        cwd         => '/opt',
        command     => 'rm -rf kibana',
        logoutput   => 'on_failure',
        onlyif      => 'test -d kibana'
    }
    ...
    archive { $url_filename:
        ...
        require         => [ Exec['remove_original_kibana_archive'] ]
    }

    # Rename the Kibana folder
    exec { 'rename_kibana_folder':
        cwd         => '/opt',
        command     => 'mv kibana-4.0.0-BETA1.1 kibana',
        logoutput   => 'on_failure',
